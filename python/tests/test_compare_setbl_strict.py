...
class TestSetblStrictParity(unittest.TestCase):
    ...
    def test_setbl_strict_parity(self):
        ...
        script = pathlib.Path(__file__).with_name("compare_setbl_strict.mjs")
        ...
        for name, py_val, js_val in (
            ("REYBL", bl.REYBL, js_results["reybl"]),
            ("RSTBL", bl.RSTBL, js_results["rstbl"]),
            ("HSTINV", bl.HSTINV, js_results["hstinv"]),
        ):
            tol = max(abs_tol, rel_tol * max(1.0, abs(js_val)))
            self.assertLessEqual(abs(py_val - js_val), tol, name)

        va_py = flatten_tensor3(ctx.VA, 3, 2, ctx.NSYS)
        vb_py = flatten_tensor3(ctx.VB, 3, 2, ctx.NSYS)
        ...
        self.assert_array_close(vb_py, js_results["vb"]["data"], abs_tol, rel_tol, "VB")
